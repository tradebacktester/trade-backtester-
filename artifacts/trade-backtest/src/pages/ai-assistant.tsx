...
function ScoreBar({ score, color }: { score: number; color: string }) {
  return (
    <div className="h-0.5 w-full rounded-full" style={{ background: "rgba(0,0,0,0.08)" }}>
      <div className="h-full rounded-full" style={{ width: `${score}%`, background: color, opacity: 0.6 }} />
    </div>
  );
}
...
function MarketAnalysisCard({ card }: { card: MarketCard }) {
  ...
  return (
    <div
      ...
      style={{ ...CARD, borderColor: open ? "rgba(0,0,0,0.16)" : C.border }}
      onClick={() => setOpen(v => !v)}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          >
            <Icon className="h-4 w-4" style={{ color: "#666" }} />
          </div>
      ...
      {open && (
        <p className="text-[11px] font-mono leading-relaxed rounded-xl px-3 py-3"
  ...
  return (
    <div className="rounded-2xl p-4 flex flex-col gap-2" style={CARD}>
      <div className="flex items-start gap-3">
        <p className="text-xs font-mono leading-relaxed flex-1" style={{ color: C.text }}>{item.title}</p>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[10px] font-mono" style={{ color: C.muted }}>{item.source} · {item.time}</span>
      </div>
    </div>
  );
}
    ...
    <div className="rounded-2xl p-4 flex flex-col gap-3" style={CARD}>
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-xs font-mono font-semibold truncate" style={{ color: C.text }}>{zone.concept}</p>
            <p className="text-[10px] font-mono" style={{ color }}>{zone.level}</p>
          </div>
        </div>
        <SentBadge s={zone.direction} />
      </div>
      <p className="text-[11px] font-mono leading-relaxed" style={{ color: C.sub }}>{zone.description}</p>
    </div>
  );
}
...
function EconRow({ ev }: { ev: EconEvent }) {
  const ICOL = { high: C.negative, medium: C.amber, low: C.positive } as const;
          ...
          <div className="flex gap-0.5">
            {levels.map((l, idx) => (
              <div key={l} className="h-1.5 w-1.5 rounded-full"
                style={{ background: levels.indexOf(ev.impact) >= idx ? color : "rgba(0,0,0,0.1)" }} />
            ))}
          </div>
...
export default function AiAssistant() {
  ...
  /* Scroll to top of page whenever tab changes */
  useEffect(() => {
  ...
  /* Helper: show/hide tab panels with CSS only — no mount/unmount = no glitch */
  function panel(id: Tab): React.CSSProperties {
    return tab === id ? {} : { display: "none" };
  }
  ...
  return (
    <div className="flex flex-col gap-4 pb-6">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div
              ...
              style={{ background: "#efefef", border: "1px solid rgba(0,0,0,0.1)" }}
            >
              <Brain className="h-5 w-5" style={{ color: "#555" }} />
            </div>
            <div className="min-w-0">
              <h1 className="text-lg sm:text-xl font-bold tracking-tight" style={{ color: C.text }}>
                AI Market Assistant
              </h1>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <span
              ...
              style={{ background: "rgba(22,163,74,0.08)", border: "1px solid rgba(22,163,74,0.2)", color: C.positive }}
            >
            ...
            >
              <RefreshCw className={`h-4 w-4 ${refreshing ? "animate-spin" : ""}`} style={{ color: C.sub }} />
            </button>
          </div>
        </div>
      </div>
      ...
      {/* ── OVERVIEW ── always in DOM, shown/hidden via CSS */}
      <div className="flex flex-col gap-3" style={panel("overview")}>
        <div className="rounded-2xl p-4 sm:p-5" style={CARD}>
          ...
          <div className="flex items-center gap-4">
            <div
              ...
              style={{ background: "#efefef", border: "1px solid rgba(0,0,0,0.09)" }}
            >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-mono font-semibold mb-1" style={{ color: overallColor }}>{overallLabel}</p>
        ...
        <p className="text-[10px] font-mono uppercase tracking-widest px-1" style={{ color: C.muted }}>Daily Market Analysis</p>
      ...
      {/* ── NEWS ── always in DOM */}
      <div className="flex flex-col gap-2" style={panel("news")}>
        <p className="text-[10px] font-mono uppercase tracking-widest px-1" style={{ color: C.muted }}>Market News</p>
        {NEWS.map((item, i) => <NewsCard key={i} item={item} />)}
      </div>

      {/* ── ICT ── always in DOM */}
      <div className="flex flex-col gap-3" style={panel("ict")}>
        <div
        ...
        <p className="text-[10px] font-mono uppercase tracking-widest px-1" style={{ color: C.muted }}>ICT Concept Analysis</p>
        <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-3" style={panel("calendar")}>
