...
import {
  TrendingUp, TrendingDown, RotateCcw, ChevronDown, ChevronUp,
  Zap, Activity, Target, DollarSign, BarChart2, Layers,
  Plus, Minus, Settings, Clock, Award, AlertCircle,
} from "lucide-react";
...
const DEMO_SYMBOLS = [
  { value: "BTCUSDT",  label: "BTC/USDT",  price: 67_420.50,  change: 2.34  },
...
function useSimPrice(base: number) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setPrice(p => {
        const delta = (Math.random() - 0.5) * 0.002 * p;
        return Math.max(0.0001, p + delta);
      });
    }, 800 + Math.random() * 400);
  ...
  return (
    <div className="flex flex-col gap-6 max-w-lg mx-auto py-4">
      <div
        ...
        style={{
          background: "linear-gradient(135deg, rgba(0,229,255,0.06) 0%, rgba(255,255,255,0.01) 100%)",
          borderColor: "rgba(0,229,255,0.2)",
          boxShadow: "0 8px 40px rgba(0,0,0,0.4), 0 0 60px rgba(0,229,255,0.05), inset 0 1px 0 rgba(0,229,255,0.1)",
        }}
      >
        <div className="absolute -top-10 -right-10 h-40 w-40 rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(0,229,255,0.15), transparent)" }} />
        <div className="relative">
          <div className="flex items-center gap-3 mb-4">
            <div className="h-12 w-12 rounded-2xl flex items-center justify-center" style={{ background: "rgba(0,229,255,0.12)", border: "1px solid rgba(0,229,255,0.25)" }}>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs font-mono">
            {[
              ...
              <div key={f.label} className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
      ...
      <div>
        <p className="text-[10px] font-mono uppercase tracking-widest mb-3 px-1" style={{ color: "hsl(220,14%,40%)" }}>Choose Starting Balance</p>
        <div className="grid grid-cols-2 gap-2">
          {BALANCE_OPTIONS.map(opt => (
            <button
              key={opt.value}
              onClick={() => setSelectedBalance(opt.value)}
              className="flex flex-col gap-1 p-4 rounded-2xl border transition-all"
              style={selectedBalance === opt.value ? {
            ...
            >
              <span className="text-xl sm:text-2xl font-bold font-mono text-left" style={{ color: selectedBalance === opt.value ? "hsl(190,90%,65%)" : "hsl(220,14%,75%)" }}>
                {opt.label}
              </span>
              <span className="text-[10px] font-mono px-2 py-0.5 rounded-full self-start" style={{ background: selectedBalance === opt.value ? "rgba(0,229,255,0.15)" : "rgba(255,255,255,0.05)", color: selectedBalance === opt.value ? "hsl(190,90%,65%)" : "hsl(220,14%,45%)" }}>
                {opt.badge}
              </span>
            </button>
          ))}
      ...
      <button
        onClick={() => onStart(selectedBalance)}
...
function MarketRow({ sym, isSelected, onClick }: {
  ...
  return (
    ...
    >
      <div>
        <p className="text-xs font-mono font-semibold" style={{ color: isSelected ? "hsl(190,90%,65%)" : "hsl(220,14%,75%)" }}>{sym.label}</p>
      </div>
      <div className="text-right">
        <p className="text-xs font-mono font-bold" style={{ color: isUp ? "hsl(150,90%,58%)" : "hsl(0,85%,62%)" }}>{price < 1 ? price.toFixed(4) : price < 100 ? price.toFixed(2) : price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
...
function TradingInterface({ initialBalance, onReset }: { initialBalance: number; onReset: () => void }) {
  ...
  const totalOpenPnl = openPositions.reduce((acc, t) => {
    const priceDiff = livePrice - t.entryPrice;
    const pnl = (t.side === "buy" ? priceDiff : -priceDiff) * t.size * t.leverage / t.entryPrice;
    return acc + pnl;
  }, 0);
  ...
  function closePosition(tradeId: number, closePrice: number) {
    ...
    if (trade) {
      ...
      const pnl = (trade.side === "buy" ? priceDiff : -priceDiff) * trade.size * trade.leverage / trade.entryPrice;
      setBalance(b => b + trade.margin + pnl);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-3 pb-24 sm:pb-8">

      {/* Stats row */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
        {statCards.map(s => (
          <div key={s.label} className="rounded-xl px-3 py-3 border flex flex-col gap-0.5" style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.07)" }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        {/* Market list */}
        <div className="rounded-2xl border p-3 flex flex-col gap-2" style={{ background: "rgba(255,255,255,0.015)", borderColor: "rgba(255,255,255,0.07)" }}>
        ...
        {/* Order panel */}
        <div className="rounded-2xl border p-4 flex flex-col gap-4" style={{ background: "rgba(255,255,255,0.015)", borderColor: "rgba(255,255,255,0.07)" }}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl font-bold font-mono" style={{ color: selectedSymbol.change >= 0 ? "hsl(150,90%,58%)" : "hsl(0,85%,62%)" }}>
                {livePrice < 1 ? livePrice.toFixed(4) : livePrice < 100 ? livePrice.toFixed(2) : livePrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </p>
            </div>
            <span className="text-xs font-mono px-2.5 py-1 rounded-xl" style={selectedSymbol.change >= 0 ? { background: "rgba(52,211,153,0.1)", color: "hsl(150,90%,58%)", border: "1px solid rgba(52,211,153,0.2)" } : { background: "rgba(239,68,68,0.1)", color: "hsl(0,85%,62%)", border: "1px solid rgba(239,68,68,0.2)" }}>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-mono font-bold px-2 py-0.5 rounded" style={{ background: "rgba(245,158,11,0.1)", color: "hsl(38,100%,65%)", border: "1px solid rgba(245,158,11,0.2)" }}>{leverage}x</span>
            </div>
            <div className="flex gap-1 flex-wrap">
              {LEVERAGE_OPTIONS.map(lev => (
                <button
                  ...
                  style={leverage === lev ? { background: "rgba(245,158,11,0.15)", color: "hsl(38,100%,65%)", border: "1px solid rgba(245,158,11,0.3)" } : { background: "rgba(255,255,255,0.04)", color: "hsl(220,14%,50%)", border: "1px solid rgba(255,255,255,0.07)" }}
                >
                  {lev}x
                </button>
              ))}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-mono font-bold" style={{ color: "hsl(220,14%,65%)" }}>{riskPct}% · {fmtUSD(riskAmount)} margin</span>
            </div>
            <input
              type="range"
              min={1}
          ...
          {margin > freeMargin && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-mono" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", color: "hsl(0,85%,65%)" }}>
          ...
          <div className="grid grid-cols-2 gap-2">
            {(["buy", "sell"] as OrderSide[]).map(side => (
              <button
                ...
                className="py-3.5 rounded-xl font-bold text-sm transition-all disabled:opacity-30"
                style={side === "buy" ? {
                  background: "linear-gradient(135deg, hsl(150,80%,28%), hsl(150,80%,22%))",
                  color: "hsl(150,90%,65%)",
                  border: "1px solid rgba(52,211,153,0.3)",
                  boxShadow: "0 4px 20px rgba(52,211,153,0.15)",
                } : {
                  background: "linear-gradient(135deg, hsl(0,70%,28%), hsl(0,70%,22%))",
                  color: "hsl(0,85%,70%)",
          ...
          <div className="flex border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
            {(["positions", "history", "analytics"] as const).map(t => (
              <button
                ...
                style={tab === t ? { color: "hsl(190,90%,65%)", borderBottom: "2px solid hsl(190,90%,65%)" } : { color: "hsl(220,14%,45%)" }}
              >
          ...
          <div className="flex-1 overflow-y-auto p-3">
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-mono px-1.5 py-0.5 rounded-full font-bold" style={t.side === "buy" ? { background: "rgba(52,211,153,0.15)", color: "hsl(150,90%,65%)" } : { background: "rgba(239,68,68,0.15)", color: "hsl(0,85%,65%)" }}>
                              {t.side.toUpperCase()}
                            </span>
                            <span className="text-xs font-mono font-semibold" style={{ color: "hsl(220,14%,75%)" }}>{DEMO_SYMBOLS.find(s => s.value === t.symbol)?.label}</span>
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px] font-mono">
                          <div className="flex justify-between"><span style={{ color: "hsl(220,14%,42%)" }}>Entry</span><span style={{ color: "hsl(220,14%,70%)" }}>${t.entryPrice.toFixed(2)}</span></div>
                          <div className="flex justify-between"><span style={{ color: "hsl(220,14%,42%)" }}>Current</span><span style={{ color: "hsl(220,14%,70%)" }}>${livePrice.toFixed(2)}</span></div>
                          <div className="flex justify-between"><span style={{ color: "hsl(220,14%,42%)" }}>Margin</span><span style={{ color: "hsl(220,14%,70%)" }}>{fmtUSD(t.margin)}</span></div>
                          <div className="flex justify-between"><span style={{ color: "hsl(220,14%,42%)" }}>P&L</span><span style={{ color: pnl >= 0 ? "hsl(150,90%,58%)" : "hsl(0,85%,62%)", fontWeight: 700 }}>{pnl >= 0 ? "+" : ""}{fmtUSD(pnl)} ({fmtPct(pnlPct)})</span></div>
                        </div>
                      </div>
                    );
                  })}
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] font-mono px-1.5 py-0.5 rounded font-bold" style={t.side === "buy" ? { background: "rgba(52,211,153,0.15)", color: "hsl(150,90%,65%)" } : { background: "rgba(239,68,68,0.15)", color: "hsl(0,85%,65%)" }}>{t.side.toUpperCase()}</span>
                          <span className="text-xs font-mono" style={{ color: "hsl(220,14%,65%)" }}>{DEMO_SYMBOLS.find(s => s.value === t.symbol)?.label}</span>
                        </div>
                        <span className="text-sm font-mono font-bold" style={{ color: (t.pnl ?? 0) >= 0 ? "hsl(150,90%,58%)" : "hsl(0,85%,62%)" }}>
            ...
            {tab === "analytics" && (
              <div className="flex flex-col gap-4 py-2">
                {[
                  { label: "Total Trades", value: closedTrades.length, color: "hsl(220,14%,75%)" },
                  ...
                  <div key={s.label} className="flex items-center justify-between px-3 py-2.5 rounded-xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)" }}>
                    <span className="text-xs font-mono" style={{ color: "hsl(220,14%,45%)" }}>{s.label}</span>
                    <span className="text-sm font-mono font-bold" style={{ color: s.color }}>{String(s.value)}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)" }}>
          <div
            ...
            style={{ background: "hsl(222,28%,10%)", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 24px 64px rgba(0,0,0,0.7)" }}
          >
            <h3 className="text-base font-bold font-mono" style={{ color: "hsl(220,14%,88%)" }}>Confirm {showConfirm === "buy" ? "Buy" : "Sell"} Order</h3>
            <div className="flex flex-col gap-2 text-xs font-mono">
              {[
                { l: "Symbol", v: selectedSymbol.label },
                { l: "Side", v: showConfirm.toUpperCase() },
                { l: "Price", v: "$" + livePrice.toFixed(2) },
                { l: "Leverage", v: leverage + "x" },
                { l: "Margin", v: fmtUSD(margin) },
              ].map(r => (
                <div key={r.l} className="flex justify-between items-center py-1.5 border-b" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowConfirm(null)} className="flex-1 py-2.5 rounded-xl text-xs font-mono transition-all" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "hsl(220,14%,55%)" }}>Cancel</button>
              <button
                onClick={() => placeOrder(showConfirm)}
                className="flex-1 py-2.5 rounded-xl text-xs font-bold font-mono transition-all"
                style={showConfirm === "buy" ? { background: "hsl(150,80%,28%)", color: "hsl(150,90%,65%)", border: "1px solid rgba(52,211,153,0.3)" } : { background: "hsl(0,70%,28%)", color: "hsl(0,85%,70%)", border: "1px solid rgba(239,68,68,0.3)" }}
              >
    ...
    return (
      <div>
        <div className="mb-4 flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl flex items-center justify-center" style={{ background: "rgba(0,229,255,0.12)", border: "1px solid rgba(0,229,255,0.2)" }}>
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "hsl(220,14%,42%)" }}>Risk-free paper trading simulator</p>
          </div>
        </div>
        <SetupScreen onStart={setStartBalance} />
      </div>
    );
  }
    ...
    <div>
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight" style={{ color: "hsl(220,14%,92%)" }}>Demo Account</h1>
            <p className="text-xs" style={{ color: "hsl(220,14%,42%)" }}>Paper trading · Simulated prices</p>
          </div>
        </div>
        <span className="flex items-center gap-1.5 text-[11px] font-mono px-2.5 py-1 rounded-full" style={{ background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.2)", color: "hsl(150,80%,55%)" }}>
          <span className="h-1.5 w-1.5 rounded-full animate-pulse" style={{ background: "hsl(150,90%,52%)" }} />
          Live Sim
        </span>
      </div>
      <TradingInterface initialBalance={startBalance} onReset={() => setStartBalance(null)} />
    </div>
  );
}
